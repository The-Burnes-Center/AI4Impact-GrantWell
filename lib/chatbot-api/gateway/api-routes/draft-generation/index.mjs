...
const STATE_NAMES = Object.fromEntries(SUPPORTED_STATES.map((s) => [s.code, s.name]));
...
export const handler = async (event) => {
  try {
    ...
    // Start the Step Functions execution. Pass userId as the trailing arg so
    // the error-path saveJobStatus preserves ownership for the status endpoint.
 ...
 */
async function startDraftGenerationExecution(jobId, input, userId) {
  ...
  try {
    ...
    // Update job status to error. Preserve userId so the owner can still
    // poll the failed job, and so the status endpoint's ownership check
    // continues to work.
    const tableName = process.env.DRAFT_GENERATION_JOBS_TABLE_NAME;
    if (tableName) {
      await saveJobStatus(jobId, {
