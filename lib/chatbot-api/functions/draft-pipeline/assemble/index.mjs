...
async function finalizeDraft(userId, sessionId, newSections, jobStatus) {
  ...
  // On total failure, just unstick the row so the user isn't trapped in
  // "generating_draft". Send them back to the questionnaire step.
  const draftStatus = jobStatus === 'error' ? 'questionnaire' : 'editing_sections';

  try {
    // Read existing draft sections so we merge instead of clobber — the user
    // may have hand-edited sections from a prior partial run.
