 ...
 *
 * Final step: merges per-section results, determines the overall job status,
 * and writes the final state to DDB.
 */
...
export const handler = async (event) => {
  ...
  console.log(`Assemble: job ${jobId} (session ${sessionId}), ${sectionResults?.length ?? 0} results`);

  // Content comes from the job row, not the Map payload (SFN 256 KB limit).
  const generatedSections = await fetchGeneratedSections(jobId);
  ...
  for (const result of sectionResults || []) {
    const content = generatedSections[result.sectionName];
    if (result.status === 'completed' && typeof content === 'string' && content.trim() !== '') {
      sections[result.sectionName] = content;
      completedCount++;
    } else {
...
async function fetchGeneratedSections(jobId) {
  const tableName = process.env.DRAFT_GENERATION_JOBS_TABLE_NAME;
  if (!tableName) return {};

  // Strongly consistent: the last generate-section writes land microseconds
  // before the Map closes, and finalizeJob overwrites `sections` wholesale with
  // whatever this read returns. A lagging replica would therefore delete prose
  // that was already persisted, and it exists nowhere else — the Map result
  // carries only section names.
  ...
  if (!result.Item?.sections) {
    console.warn(`Assemble: job ${jobId} has no stored sections`);
    return {};
  }
  return unmarshall({ sections: result.Item.sections }).sections || {};
}
...
async function finalizeDraft(userId, sessionId, newSections, jobStatus) {
  ...
  try {
    ...
    // Strongly consistent for the same reason: this is a read-modify-write of
    // `sections`, so a stale answer reverts the user's latest autosave, and one
    // that misses the row drops every section they wrote by hand.
    ...
    const updates = {
      ...
      last_write_source: 'ai_generated',
    };
    ...
    // Unconditional bump so revs stay coherent across both writers.
    updateExpressions.push('#rev = if_not_exists(#rev, :revZero) + :revOne');
    expressionAttributeNames['#rev'] = 'rev';
    expressionAttributeValues[':revZero'] = 0;
    expressionAttributeValues[':revOne'] = 1;
