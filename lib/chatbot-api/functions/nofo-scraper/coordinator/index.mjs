...
export const handler = async (event) => {
  ...
  // requireAdmin returns null when requestContext is absent, so scheduled
  // EventBridge invocations pass through unchanged.
  ...
  try {
    while (true) {
      ...
      const normalizedTitles = opportunities.map((o) => normalizeNofoFolderName(o.opportunity_title));
      let existingMap;
      try {
        existingMap = await batchGetMetadata(normalizedTitles);
      } catch (err) {
      ...
      for (const opp of opportunities) {
        ...
        const existing = existingMap.get(normalizeNofoFolderName(oppTitle));
