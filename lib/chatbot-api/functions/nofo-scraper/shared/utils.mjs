...
export const CLAUDE_MODEL_ID = process.env.HAIKU_MODEL_ID;
...
export function normalizeNofoFolderName(name) {
  return (name || '')
    // Source titles carry non-breaking spaces and other unicode whitespace.
    // Untrimmed, "Foo " and "Foo" become distinct records/folders,
    // producing duplicate grants and false self-duplicate quarantines.
    .replace(/[  -   　﻿]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .replace(/\//g, '-');
