...
export const handler = async (event) => {
  ...
  for (const record of event.Records) {
    ...
    try {
      ...
      // --- Dedup guard ---
      const existingItem = await getExistingItem(opportunityTitle);
      if (existingItem) {
        if (!isUpdate) {
          console.log(`Dedup: "${opportunityTitle}" already exists, skipping new processing`);
          continue;
        }
        const storedUpdatedAt = existingItem.source_updated_at || null;
        if (apiUpdatedAt && storedUpdatedAt && storedUpdatedAt >= apiUpdatedAt) {
          console.log(`Dedup: "${opportunityTitle}" already up-to-date, skipping`);
          continue;
        }
      }
      ...
      if (extension === 'html') {
        s3Key = `pending-conversion/${details.opportunity_title}/NOFO-File-HTML.html`;
        contentType = 'text/html';
      } else {
        s3Key = `${details.opportunity_title}/NOFO-File-PDF`;
      ...
      if (details.category) {
        await writeMetadata(details.opportunity_title, details, isUpdate, newHash);
      }
