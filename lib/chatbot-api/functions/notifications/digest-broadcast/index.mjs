/**
 * Developer-only digest trigger. Fires the REAL digest on demand (async-invokes the digest Lambda)
 * instead of waiting for the cron — real send, not a [TEST] preview, so it's exactly what a user
 * receives (real subject, working unsubscribe link, watermark advance).
 *
