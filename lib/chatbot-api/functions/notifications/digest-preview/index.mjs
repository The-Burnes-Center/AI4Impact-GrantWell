...
export const handler = async (event) => {
  ...
  try {
    ...
    const rendered = buildDigest(matches, frequency, {
      appUrl: APP_URL,
      branding: BRANDING,
    ...
    if (method === "POST") {
      if (matches.length === 0) {
        // Nothing matched the caller's prefs this window — send nothing (the real digest skips too).
        return respond(200, {
          sent: false,
          count: 0,
          message: "No matching grants right now — nothing to send.",
        });
      }
      ...
      return respond(200, {
        ...
        message: `Test digest (${matches.length} grant${matches.length === 1 ? "" : "s"}) sent to ${email}`,
      });
    }

    return respond(200, { rendered, count: matches.length });
  } catch (error) {
