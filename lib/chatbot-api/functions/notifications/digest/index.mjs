...
// Admin-editable digest copy lives in a singleton CONFIG row (same pattern as feature rollouts).
const DIGEST_COPY_KEY = { featureKey: "notification-digest", subjectKey: "CONFIG" };

async function getDigestCopy() {
  if (!CONFIG_TABLE) return {};
  try {
    const res = await dynamoClient.send(
      new GetItemCommand({
        TableName: CONFIG_TABLE,
        Key: marshall(DIGEST_COPY_KEY),
      })
    );
    if (!res.Item) return {};
    ...
    console.error("Could not load digest copy:", error);
    return {};
  }
}
...
export const handler = async (event) => {
  ...
  const copy = await getDigestCopy();
...
function matchesPrefs(nofo, prefs) {
  // State is fixed to the user's assigned state: a NOFO scoped to a different
  // state is never in a user's digest. A NOFO with no state is treated as national
  // and always passes the state gate.
  const userState = String(prefs.state || "").toUpperCase();
  const nofoState = String(nofo.state || "").toUpperCase();
  if (userState && nofoState && nofoState !== userState) return false;

  const categories = arr(prefs.categories);
  const keywords = arr(prefs.keywords);

  // No topic filters → every new active NOFO in the user's state.
  if (!categories.length && !keywords.length) return true;
