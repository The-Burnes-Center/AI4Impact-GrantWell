...
export const handler = async (event) => {
  ...
  // `onlyUserId` targets a single user (the "send to me" / developer path): they get the exact same
  // real digest — real subject, real unsubscribe link, watermark advance — regardless of the cadence
  // stored in their prefs. Absent it, this is the normal cron run over everyone on this frequency.
  const users = event?.onlyUserId
    ? await getPrefsForUser(event.onlyUserId)
...
async function getPrefsForUser(userId) {
  // Single-user targeted send: return the user's prefs row so they get their real digest even if
  // their stored frequency wouldn't include them in this cadence's cron run. Empty if no prefs saved.
  const res = await dynamoClient.send(
    new GetItemCommand({
      TableName: PREFS_TABLE,
