...
export function computeQualityScore(summary) {
  ...
  for (const [field, weight] of Object.entries(weights)) {
    ...
    if (Array.isArray(items) && items.length > 0) {
      // Presence score: 1 item gets 70%, 2+ items gets 100% of the base portion
      const presenceScore = Math.min(0.7 + (items.length - 1) * 0.3, 1);
      const baseScore = presenceScore * weight * 0.6;
      // Confidence bonus: proportion of high-confidence items
      const highConfCount = items.filter((i) => i.confidence === "high").length;
      const confidenceBonus = (highConfCount / items.length) * weight * 0.4;
