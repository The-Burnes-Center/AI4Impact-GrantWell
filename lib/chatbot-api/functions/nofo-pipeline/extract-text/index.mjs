...
export const handler = async (event) => {
  ...
  if (documentContent.length > 0) {
    ...
    const lowerContent = documentContent.substring(0, 3000).toLowerCase();
    const matchCount = grantKeywords.filter((kw) => lowerContent.includes(kw)).length;
    if (matchCount < 2) {
  ...
  const contentHash = createHash("sha256")
    .update(documentContent.substring(0, 5000))
    .digest("hex");
  ...
  if (tableName) {
    try {
      ...
      const matches = (scanResult.Items || []).map((item) => unmarshall(item));
