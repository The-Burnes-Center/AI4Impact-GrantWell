...
export const handler = async (event) => {
  ...
  // Only dedup real documents. A failed-quality doc (stub / "Loading…" shell)
  // hashes to the same junk as every other stub, so running dedup on it produces
  // false "duplicate of X" matches between unrelated grants.
  if (tableName && !sourceQualityFailed) {
    try {
