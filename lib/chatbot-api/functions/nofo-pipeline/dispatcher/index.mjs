  ...
  for (const sqsRecord of event.Records) {
    try {
      ...
      for (const record of s3Event.Records || []) {
        ...
        // Put the record into the `processing` StatusIndex up front so the admin Grants list shows
        // the row *while it processes* — that's what surfaces the progress stepper. `processing` is
        // deliberately NOT `active`: the user-facing list only returns status='active', so an
        // in-flight NOFO is structurally invisible to users. publish flips it to `active` at the
        // end; quarantine leaves it out of the active set.
