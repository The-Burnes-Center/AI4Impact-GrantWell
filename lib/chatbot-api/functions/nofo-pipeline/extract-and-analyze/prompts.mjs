...
CRITICAL: Only extract eligibility criteria that are EXPLICITLY STATED in the NOFO text. NEVER infer, assume, or fabricate eligibility information based on your knowledge of the funding agency, program type, or general federal grant policies. If the NOFO does not explicitly list eligible entity types, do NOT create an "Eligible Entity Types" item — omit it entirely. Descriptions like "The NOFO does not explicitly specify..." or "Based on [agency]'s general policies, eligible entities typically include..." are STRICTLY FORBIDDEN. Every item must be directly supported by a verbatim quote from the NOFO.

Thematic groupings to look for:
- Eligible entity/applicant types (combine ALL eligible organizations, institutions, individuals, and group applicants into ONE item — ONLY if explicitly stated in the NOFO)
...
Only create an item when the NOFO EXPLICITLY contains relevant content for that theme. If a category is not addressed in the NOFO, omit it — do NOT fill it in with assumed or inferred information. Descriptions must focus on actionable details, conditions, and exceptions — never restate the item name.
...
## OUTPUT INSTRUCTIONS
NEVER infer, assume, or fabricate information based on your knowledge of the funding agency or general grant policies. If something is not explicitly stated in the NOFO, omit it. Every extracted item MUST be directly supported by text that actually appears in the document. An empty array is the correct output when the NOFO does not address a category.

For each item provide:
- item: The requirement/document/section name
- description: 2-3 sentence explanation
- confidence: "high", "medium", or "low" based on clarity of source text
- sourceQuote: A brief verbatim quote from the NOFO supporting this item (MUST be an actual quote from the document, not a paraphrase or inference)
