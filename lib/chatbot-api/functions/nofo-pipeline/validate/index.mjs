...
export const handler = async (event) => {
  ...
  if (!hasEligibility && !hasNarrative) {
    ...
    // Some non-critical categories missing — only escalate if a critical section is missing
    const missingCritical = !hasEligibility || !hasNarrative;
    if (missingCritical && validationResult.overallVerdict === "PASS") {
  ...
  // --- Auto-approve if both critical sections are present ---
  if (
    hasEligibility &&
    hasNarrative &&
    criticalCount === 0 &&
    (validationResult.overallVerdict === "NEEDS_REVIEW" || validationResult.overallVerdict === "PASS")
  ) {
    validationResult.overallVerdict = "PASS";
    console.log(
      `Auto-approved ${nofoName}: both critical sections present, no critical issues ` +
      `(score=${validationResult.qualityScore}, warnings=${warningCount}, info=${infoCount})`
    );
  }
