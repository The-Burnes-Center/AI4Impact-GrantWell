...
export const handler = async (event) => {
  try {
    ...
    requirePlatformAdmin(userContext.roles, userContext.state);
...
// State admins (Admin + custom:state) can't manage users — prevents them from
// creating peer admins or reassigning users into/out of their state.
function requirePlatformAdmin(roles, state) {
  const isDeveloper = hasRole(roles, "Developer");
  const isRegularAdmin = hasRole(roles, "Admin") && !state;
  if (!isDeveloper && !isRegularAdmin) {
    const error = new Error(
      "Only Developers and platform administrators (Admins without a state) can manage users."
    );
