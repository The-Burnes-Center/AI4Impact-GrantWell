...
// Platform authority is the explicit PlatformAdmin role. Pre-migration accounts (Admin with
// no state) still count while this flag is on; it must stay on until every pool is migrated.
const LEGACY_STATELESS_ADMIN_IS_PLATFORM =
  String(process.env.LEGACY_STATELESS_ADMIN_IS_PLATFORM ?? "true").toLowerCase() !== "false";

function isPlatformAdmin(roles, state) {
  if (hasRole(roles, "PlatformAdmin")) return true;
  return LEGACY_STATELESS_ADMIN_IS_PLATFORM && hasRole(roles, "Admin") && !state;
}

// SUPPORTED_STATES is serialized as [{code,name},...]; extract the codes.
...
function normalizeStoredStateCode(raw) {
  ...
  return LEGACY_NAME_TO_CODE[trimmed.toLowerCase()] || "";
}
...
async function handleUpdate(scope, actor, event) {
  ...
  // PlatformAdmin is inherently stateless; a leftover state claim would otherwise resolve
  // the account to stateAdmin and silently narrow its authority.
  if (nextRoles?.includes("PlatformAdmin") && nextState) {
    return respond(400, {
      message: "Platform administrators cannot be scoped to a state",
    });
  }
  if (nextRoles?.includes("PlatformAdmin") && nextState === undefined && target.state) {
    nextState = "";
    userAttributes.push({ Name: "custom:state", Value: "" });
  }
...
function getActorScope(actor) {
  ...
  if (isPlatformAdmin(actor.roles, actor.state)) {
    return { kind: "platformAdmin", state: "" };
  }
...
function assertCanManageTarget(scope, target) {
  ...
  // Only a developer may manage another developer or a platform admin.
  if (hasRole(target.roles, "Developer")) {
    const error = new Error("Only developers can manage developer accounts");
    error.statusCode = 403;
    throw error;
  }
  if (hasRole(target.roles, "PlatformAdmin")) {
    const error = new Error("Only developers can manage platform administrator accounts");
    error.statusCode = 403;
    throw error;
  }
...
function requireRoleAssignmentPermission(scope, rolePreset) {
  ...
  // Platform admin is unrestricted cross-state authority, so it is developer-grantable only:
  // a state admin must not be able to mint one and escape their own scoping.
  if (rolePreset === "platformadmin" && scope.kind !== "developer") {
    const error = new Error("Only developers can assign platform administrator access");
    error.statusCode = 403;
    throw error;
  }
}
...
async function assertNotLastPrivilegedUser(target, nextRoles) {
  ...
  // The target keeps platform authority only if the next roles still grant it. nextState is
  // unknown here, so treat a retained bare Admin as platform-holding while legacy is on.
  const keepsPlatformAdmin =
    nextRoleSet.has("PlatformAdmin") ||
    (LEGACY_STATELESS_ADMIN_IS_PLATFORM && nextRoleSet.has("Admin"));
  const losesPlatformAdmin =
    isPlatformAdmin(target.roles, target.state) && !keepsPlatformAdmin;
  ...
  do {
    ...
    for (const user of response.Users || []) {
      ...
      if (hasRole(mapped.roles, "Developer")) {
        developers += 1;
      } else if (isPlatformAdmin(mapped.roles, mapped.state)) {
        platformAdmins += 1;
      }
    }
