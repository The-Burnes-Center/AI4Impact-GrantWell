...
export const handler = async (event) => {
  try {
    ...
    if (rawPath === "/feature-rollouts/me" && method === "GET") {
      const [searchAccess, maintenanceAccess] = await Promise.all([
        getFeatureAccess("ai-grant-search", userContext.email),
        getFeatureAccess("maintenance-mode", userContext.email),
      ]);
      return respond(200, {
        ...
        features: {
          aiGrantSearch: searchAccess,
          maintenanceMode: maintenanceAccess,
        },
      });
    }
...
async function searchUsers(query, roleFilter) {
  ...
  const filterParams = trimmedQuery
...
function normalizeRoleFilter(value) {
  ...
  if (["admin", "developer"].includes(normalized)) {
    return normalized;
  }
  if (normalized === "user") {
    return "user";
