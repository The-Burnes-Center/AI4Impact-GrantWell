...
function validatePrefs(body) {
  ...
  return {
    frequency,
    // states stored uppercased to match NOFO metadata + custom:state convention
    states: stringArray(body.states).map((s) => s.toUpperCase()),
...
async function putPrefs(userId, prefs) {
  // Read-modify-write preserves last_sent (set only by the digest Lambda), which the
  // client never sends.
