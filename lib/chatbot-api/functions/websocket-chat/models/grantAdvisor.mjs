  ...
  async processQuery(connectionId, query, userPreferences = {}) {
    try {
      ...
      const grantRecommendations = body.grants.map(grant => {
        // Extract the real grant name from grantId if needed
        // The grantId might be in the format of "agency-name-year-id"
        // We want to make it more readable if there's no proper name provided
        let grantName = grant.name;
        if (grantName === grant.grantId || !grantName || grantName.startsWith('Grant ')) {
        ...
        return {
          id: grant.name,
          name: grantName, // Use the improved name
