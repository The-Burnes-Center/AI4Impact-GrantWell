...
async function retrieveKBDocs(query, knowledgeBase, knowledgeBaseID, documentIdentifier, userId) {
  // Build metadata filter expression for Bedrock KB
  // This filters at query time for better privacy and performance
  const filterConditions = [];
  
  // Condition 1: NOFO documents with matching documentIdentifier
  filterConditions.push({
    and: [
      ...
      {
        equals: {
          key: "metadataAttributes.documentIdentifier",
  ...
  // Condition 2: User documents with matching userId (if provided)
  if (userId) {
    filterConditions.push({
      and: [
        {
  ...
  // Combine conditions with OR logic - return NOFO docs OR user's docs
  ...
  const input = {
    ...
    retrievalQuery: { 
      text: query,
      // Add metadata filter for query-time filtering
  ...
  try {
    ...
    console.log(`KB query with metadata filter - documentIdentifier: ${documentIdentifier}, userId: ${userId || 'none'}`);

    // Results are already filtered by metadata at KB level
    // Still apply confidence filtering and URI validation as fallback
    const folderPath = documentIdentifier.endsWith("/") ? documentIdentifier : `${documentIdentifier}/`;
    const userFolderPath = userId ? `${userId}/` : null;
    ...
    const confidenceFilteredResults = response.retrievalResults.filter(item => {
      try {
        ...
        // Additional URI-based validation as fallback (for documents without metadata)
        // This ensures backward compatibility with documents that don't have metadata yet
        const matchesNofo = uri.includes(folderPath);
        const matchesUserDocs = userFolderPath && uri.includes(userFolderPath);
        
        // If metadata filtering worked, this should always pass, but keep as safety check
        return matchesNofo || matchesUserDocs;
      } catch (error) {
    ...
    // If metadata filter fails, try fallback without filter (for backward compatibility)
    if (error.name === 'ValidationException' || error.message?.includes('filter')) {
      ...
      try {
        ...
        const fallbackCommand = new KBRetrieveCommand(fallbackInput);
        const fallbackResponse = await knowledgeBase.send(fallbackCommand);
        
        // Apply URI-based filtering as fallback
        const folderPath = documentIdentifier.endsWith("/") ? documentIdentifier : `${documentIdentifier}/`;
        const userFolderPath = userId ? `${userId}/` : null;
