  ...
  try {
    ...
    if (userDocFilter) {
      const [nofoResult, userResult] = await Promise.all([
        ...
        runKBQuery(query, knowledgeBase, knowledgeBaseID, userDocFilter, 5, uri => uri.includes(userFolderPath)),
      ]);
      ...
      try {
        ...
        const fallbackResults = (fallbackResponse.retrievalResults || []).filter(item => {
          try {
            const uri = item.location?.s3Location?.uri;
            if (!uri || item.score <= 0.5) return false;
            return uri.includes(folderPath) || (userFolderPath && uri.includes(userFolderPath));
          } catch {
            return false;
          }
        });
