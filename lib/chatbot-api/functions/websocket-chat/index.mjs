...
/**
 * Resolves the caller's `custom:state` (a 2-letter code) by hitting Cognito.
 * Per-message lookup with no caching — a chat message takes seconds end-to-end,
 * so the ~50-150ms AdminGetUser is acceptable. If staging chat traffic grows,
 * cache by connectionId at $connect time instead.
 *
 * Returns "" (empty) on any failure or unrecognized value, which combined with
 * the buildScopeFilter logic gives federal-only retrieval (fail-closed).
 */
async function resolveUserState(userId) {
  if (!userId || !process.env.USER_POOL_ID) return "";
  try {
    ...
    const attr = (result.UserAttributes || []).find(a => a.Name === "custom:state");
    const raw = (attr?.Value || "").trim().toUpperCase();
    return SUPPORTED_STATE_CODES.has(raw) ? raw : "";
  } catch (err) {
    console.warn(`Could not resolve custom:state for user ${userId}:`, err.message);
    return "";
  }
}

/**
 * Returns the scope sub-filter (federal OR (state AND state=userState)) to
 * AND-combine with the existing per-NOFO filters. When the feature flag is
 * off OR the user has no resolvable state, returns null and callers skip
 * scope filtering.
 */
function buildScopeFilter(userState) {
  if (!NOFO_STATE_FILTER_ENABLED) return null;
  if (!userState) {
    // Fail closed to federal-only.
...
async function retrieveKBDocs(query, knowledgeBase, knowledgeBaseID, documentIdentifier, userId, scopeFilter) {
  ...
  // NOFO filter: documentType + documentIdentifier, optionally AND-combined
  // with the per-state scope filter when the feature flag is on. We keep the
  // existing `and` keyword for backward-compat with the previously-deployed
  // shape; if that turns out to be a no-op (Bedrock's documented operator is
  // `andAll`), the URI filter inside runKBQuery still narrows by folderPath.
  ...
  // User-uploaded documents are always user-scoped (they're the caller's own
  // files), so no need to apply the state scope filter here.
...
const getUserResponse = async (id, requestJSON) => {
  try {
    ...
    // State is resolved server-side from Cognito, never trusted from the
    // request body. Used both for prompt interpolation (so the model knows
    // the user's state) and the per-state KB scope filter (when enabled).
