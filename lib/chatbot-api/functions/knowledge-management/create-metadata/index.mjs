...
export const handler = async (event) => {
  ...
  for (const record of event.Records || []) {
    ...
    const sourcesToSync = [];
    if (metadataCreatedForUserDocs && process.env.USER_DOCUMENTS_SOURCE) {
      sourcesToSync.push({ syncSource: 'user-documents', dataSourceId: process.env.USER_DOCUMENTS_SOURCE });
    }
    if (metadataCreatedForNofos && process.env.SOURCE) {
      sourcesToSync.push({ syncSource: 'nofo', dataSourceId: process.env.SOURCE });
    }

    for (const source of sourcesToSync) {
      try {
        const alreadyRunning = await isSyncRunning(kbId, source.dataSourceId);
        if (alreadyRunning) {
          console.log(`KB sync already in progress for ${source.syncSource}, skipping`);
          continue;
        }
        console.log(`Triggering KB sync for ${source.syncSource}...`);
        await lambdaClient.send(new InvokeCommand({
          FunctionName: process.env.SYNC_KB_FUNCTION_NAME,
          InvocationType: 'Event',
          Payload: JSON.stringify({ syncSource: source.syncSource }),
        }));
        console.log(`KB sync triggered for ${source.syncSource}`);
      } catch (error) {
        console.error(`Failed to trigger KB sync for ${source.syncSource}:`, error);
      }
    }
  }
