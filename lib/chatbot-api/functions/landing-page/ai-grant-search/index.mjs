...
export const handler = async (event) => {
  ...
  try {
    ...
    // Merge: hybrid results first, then category matches, then name matches
    const mergedSet = new Set(hybridResults.map((r) => r.name));

    const categoryOnly = categoryNames
      .filter((name) => !mergedSet.has(name))
      .map((name) => { mergedSet.add(name); return { name, score: CATEGORY_BOOST_SCORE, source: 'category', reason: `In category: ${matchedCategory}` }; });

    const nameOnly = nameMatches
      .filter((name) => !mergedSet.has(name))
      .map((name) => ({ name, score: NAME_MATCH_SCORE, source: 'name', reason: 'Matched by grant name' }));

    if (categoryOnly.length > 0) console.log(`Category boost: added ${categoryOnly.length} grants from "${matchedCategory}"`);
    if (nameOnly.length > 0) console.log(`Name boost: added ${nameOnly.length} grants matching "${trimmedQuery}" in name`);

    const finalResults = [...hybridResults, ...categoryOnly, ...nameOnly].slice(0, MAX_RESULTS);
  ...
  try {
    const result = await dynamoClient.send(
      new ScanCommand({
        TableName: METADATA_TABLE,
        ProjectionExpression: 'nofo_name',
        FilterExpression: 'contains(nofo_name, :q)',
        ExpressionAttributeValues: { ':q': { S: lowerQuery } },
      })
    );
    const scanNames = (result.Items || []).map((item) => unmarshall(item).nofo_name).filter(Boolean);
    if (scanNames.length > 0) return scanNames;

    const fullResult = await dynamoClient.send(
      new ScanCommand({ TableName: METADATA_TABLE, ProjectionExpression: 'nofo_name' })
    );
    return (fullResult.Items || [])
      .map((item) => unmarshall(item).nofo_name)
