...
async function hybridSearch(query, vector, k) {
  ...
  try {
    ...
    const bm25MatchClause = wordCount <= 3
      ? { query, operator: 'and' }
      : { query, operator: 'or', minimum_should_match: '60%' };
...
async function detectFilterType(searchTerm, tableName) {
  ...
  if (!matchedAgency) {
    const agencyPattern = new RegExp(`\\b${escapeRegExp(normalizedSearch)}\\b`, 'i');
    matchedAgency = agencies.find(a => agencyPattern.test(a));
  }

  if (!matchedCategory) {
    const categoryPattern = new RegExp(`\\b${escapeRegExp(normalizedSearch)}\\b`, 'i');
    matchedCategory = categories.find(c => categoryPattern.test(c));
  }
