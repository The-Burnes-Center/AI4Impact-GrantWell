...
const SCORE_THRESHOLD_SHORT = 0.50;
const SCORE_THRESHOLD_LONG = 0.40;
...
const KNN_FETCH_MULTIPLIER = 3; // Fetch more chunks for better aggregation

const bedrockClient = new BedrockRuntimeClient({ region: REGION });
const dynamoClient = new DynamoDBClient({ region: REGION });

// ── SigV4 signing helpers (Node.js crypto, zero external deps) ──
...
// ── Hybrid search via native AOSS requests ──
...
async function hybridSearch(query, vector, k) {
  ...
  // B1c: Dynamic threshold based on query length
  const scoreThreshold = wordCount <= SHORT_QUERY_WORD_LIMIT
    ? SCORE_THRESHOLD_SHORT
    : SCORE_THRESHOLD_LONG;

  try {
    // B4b: OR + 80% minimum_should_match for all queries
    const bm25MatchClause = { query, operator: 'or', minimum_should_match: BM25_MIN_SHOULD_MATCH };

    // B5b: Fetch more k-NN chunks for per-NOFO aggregation
    ...
    // B3b: Saturation normalization for BM25 — score / (score + k)
    ...
    for (const hit of knnHits) {
      const name = extractNofoName(hit);
      if (!name) continue;
      const chunkScore = Math.max(0, Math.min(1, hit._score || 0));
    ...
    // A1: Per-result score breakdown logging
