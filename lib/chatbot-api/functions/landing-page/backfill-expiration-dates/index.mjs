...
export const handler = async (event, context) => {
  ...
  // Parse startIndex from event if resuming
  const eventBody = typeof event.body === 'string' ? JSON.parse(event.body) : event;
  const startIndex = eventBody?.startIndex || 0;
  const TIMEOUT_BUFFER_MS = 30000; // Stop 30 seconds before timeout
  ...
  try {
    ...
    console.log(`Starting backfill (dryRun: ${dryRun}, startIndex: ${startIndex})...`);
    ...
    console.log(`Found ${allFolders.length} total NOFO folders in S3`);
    
    // Process folders starting from startIndex
    const foldersToProcess = allFolders.slice(startIndex);
    let currentIndex = startIndex;

    // Step 2: Process each folder with timeout checking
    for (const folderName of foldersToProcess) {
      // Check remaining time before processing each NOFO
      if (context && context.getRemainingTimeInMillis) {
        const remainingTime = context.getRemainingTimeInMillis();
        if (remainingTime < TIMEOUT_BUFFER_MS) {
          console.log(`\n⚠️ Stopping early: ${remainingTime}ms remaining (less than ${TIMEOUT_BUFFER_MS}ms buffer)`);
          break;
        }
      }
      ...
      try {
        console.log(`\n[${currentIndex}/${allFolders.length}] Processing: ${folderName}`);
    ...
    const isComplete = currentIndex >= allFolders.length;
    const nextStartIndex = isComplete ? null : currentIndex;

    console.log('\n========================================');
    console.log(isComplete ? 'BACKFILL COMPLETED' : 'BATCH COMPLETED (more remaining)');
    console.log('========================================');
    console.log(`Processed: ${stats.totalProcessed} NOFOs (${startIndex} to ${currentIndex - 1})`);
    ...
    if (!isComplete) {
      console.log(`\n⚠️ More NOFOs remaining. Next startIndex: ${nextStartIndex}`);
      console.log(`Invoke again with: { "startIndex": ${nextStartIndex} }`);
    }
    ...
    return {
      ...
      body: JSON.stringify({
        message: isComplete ? (dryRun ? 'Dry run completed' : 'Backfill completed') : 'Batch completed, more remaining',
        isComplete,
        nextStartIndex,
        stats: {
