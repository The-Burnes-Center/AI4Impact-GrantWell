...
async function analyzeAndGenerateSections(query, grantInfos, projectBasics, questionnaire, documentIdentifier, sessionId) {
  ...
  const prompt = `<role>
You are an expert grant writing assistant. Generate a comprehensive, personalized grant application draft based on the inputs below.
</role>
...
<instructions>
Write a complete grant application draft. For each required NOFO section:
1. Address the user's query where relevant
2. Personalize content using specific details from project_basics and questionnaire_responses
3. Integrate requirements and best practices from the grant_knowledge_base
4. Align with the grant's focus areas, priorities, and evaluation criteria
5. Write specific, evidence-based content — not generic statements
6. Follow the section descriptions exactly as specified in the NOFO
7. Maintain consistent project goals, methodology, and outcomes across all sections
8. Use professional grant writing language and tone
9. Include measurable outcomes and evaluation methods where applicable
</instructions>

<output_rules>
- Output ONLY a valid JSON object — no preamble, explanation, or trailing text
- Each key must be the exact section title from the NOFO <title> tags
- Each value must be the complete, well-developed content for that section
- Maintain section order as specified in the NOFO
- Include ALL required sections; do not add sections not listed in the NOFO
</output_rules>

Provide your JSON response immediately:`;
  
  const sectionProperties = {};
  const requiredSections = [];
  for (const section of summary.ProjectNarrativeSections) {
    sectionProperties[section.item] = {
      type: "string",
      description: `Content for: ${section.description}`,
    };
    requiredSections.push(section.item);
  }
  ...
  try {
    const sections = await invokeStructuredOutput(bedrockClient, {
