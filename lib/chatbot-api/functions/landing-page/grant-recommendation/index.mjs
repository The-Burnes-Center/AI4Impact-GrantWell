...
export const handler = async (event) => {
  ...
  try {
    ...
    console.log(`\n========== GRANT RECOMMENDATION SEARCH START ==========`);
    console.log(`[Search] Query: "${query}"`);
    console.log(`[Search] User Preferences:`, JSON.stringify(userPreferences));
    console.log(`[Search] Start Time: ${new Date().toISOString()}`);
    
    // Generate a unique job ID for this search
    const jobId = randomUUID();
    console.log(`[Job ${jobId}] Starting grant recommendation search`);
    ...
    console.log(`[Timing] Filtered grants search completed in ${filterDuration}ms`);
    ...
    // Generate response with filtered grants only (RAG will come via polling)
    const responseStartTime = Date.now();
    const response = await generateRecommendationResponse(query, filteredGrants);
    const responseDuration = Date.now() - responseStartTime;
    console.log(`[Timing] Response generation completed in ${responseDuration}ms`);
    ...
    response.ragCount = 0;
    response.ragStatus = 'pending';
    response.filters = filters;
    
    const totalDuration = Date.now() - startTime;
    console.log(`[Timing] Initial response time: ${totalDuration}ms`);
    console.log(`[Job ${jobId}] Returning ${filteredGrants.length} filtered grants, starting async RAG search`);
    console.log(`========== GRANT RECOMMENDATION SEARCH STARTED ==========\n`);
    
    // Start async RAG search
    const functionName = process.env.AWS_LAMBDA_FUNCTION_NAME;
    if (functionName && searchJobsTable) {
      try {
        const invokeParams = {
        ...
        const invokeCommand = new LambdaInvokeCommand(invokeParams);
        await lambdaClient.send(invokeCommand);
        console.log(`[Job ${jobId}] Started async RAG search`);
      } catch (invokeError) {
        console.error(`[Job ${jobId}] Error invoking async RAG search:`, invokeError.message);
        // Update job status to error
        if (searchJobsTable) {
          await saveJobStatus(jobId, {
            ragStatus: 'error',
            ragError: invokeError.message
          });
        }
      }
    }
 ...
 * Combines filtered grants + RAG grants, deduplicates, and saves complete results
 */
async function handleAsyncRagSearch(event) {
  ...
  console.log(`\n========== ASYNC RAG SEARCH START ==========`);
  console.log(`[Job ${jobId}] Starting RAG search`);
  console.log(`[Job ${jobId}] Filtered grants: ${(filteredGrants || []).length}`);
  ...
  try {
    ...
    console.log(`[Timing] RAG search completed in ${ragDuration}ms`);
    console.log(`[Job ${jobId}] Found ${ragGrants.length} RAG grants`);
    
    // Combine filtered and RAG grants with explicit deduplication
    const dedupStartTime = Date.now();
    const normalizeGrantId = (id) => {
      if (!id) return null;
    ...
    // Add filtered grants first (they take priority)
    for (const grant of (filteredGrants || [])) {
      const grantId = grant.id || grant.grantId;
      const normalizedId = normalizeGrantId(grantId);
      if (normalizedId && !grantMap.has(normalizedId)) {
        grantMap.set(normalizedId, grant);
      }
    }
    
    // Add RAG grants, skipping duplicates
    let duplicateCount = 0;
    for (const grant of ragGrants) {
      const grantId = grant.id || grant.grantId;
      const normalizedId = normalizeGrantId(grantId);
      if (normalizedId && !grantMap.has(normalizedId)) {
        grantMap.set(normalizedId, grant);
    ...
    const allGrants = Array.from(grantMap.values());
    const dedupDuration = Date.now() - dedupStartTime;
    
    if (duplicateCount > 0) {
      console.log(`[Deduplication] Removed ${duplicateCount} duplicate grant(s) when combining filtered and RAG results`);
    }
    console.log(`[Timing] Deduplication completed in ${dedupDuration}ms`);
    console.log(`[Results] Total unique grants: ${allGrants.length} (${(filteredGrants || []).length} filtered + ${ragGrants.length} RAG - ${duplicateCount} duplicates)`);
    
    // Update job with complete deduplicated results
    if (searchJobsTable) {
      await saveJobStatus(jobId, {
        status: 'completed',
        filteredGrants: filteredGrants || [],
        ragGrants: ragGrants,
        allGrants: allGrants, // Complete deduplicated results
        ragStatus: 'completed',
    ...
    const totalDuration = Date.now() - startTime;
    console.log(`[Timing] Total async RAG processing time: ${totalDuration}ms`);
    console.log(`[Job ${jobId}] Completed - ${allGrants.length} total unique grants ready`);
    console.log(`========== ASYNC RAG SEARCH COMPLETE ==========\n`);
    
    return { success: true, ragGrantsCount: ragGrants.length, totalGrantsCount: allGrants.length };
  } catch (error) {
    console.error(`[Job ${jobId}] RAG search error:`, error.message);
    const totalDuration = Date.now() - startTime;
    console.log(`[Timing] RAG search failed after ${totalDuration}ms`);
...
async function getFilteredGrantsImmediate(query, userPreferences = {}) {
  ...
  if (detectDuration > 0) {
    console.log(`[Timing] Filter detection completed in ${detectDuration}ms`);
  }
  ...
  if (filters.category && tableName) {
    ...
    console.log(`[Timing] Category search completed in ${categoryDuration}ms`);
  ...
  if (filters.agency && tableName) {
    ...
    const agencyDuration = Date.now() - agencyStartTime;
    console.log(`[Timing] Agency search completed in ${agencyDuration}ms`);
  ...
  if (tableName && query && query.length >= 3) {
    ...
    console.log(`[Timing] Name search completed in ${nameDuration}ms`);
  ...
  console.log(`[Timing] Summary fetching completed in ${summaryDuration}ms`);
  
  const totalDuration = Date.now() - functionStartTime;
  console.log(`[Immediate Filter] Returning ${enhancedFilteredGrants.length} grants (no LLM scoring - direct match)`);
  console.log(`[Timing] Total filter search time: ${totalDuration}ms`);
...
async function findMatchingGrantsWithRAGOnly(query, userPreferences = {}, filters = {}, existingGrants = []) {
  ...
  // Normalize grant IDs for consistent comparison (remove trailing slashes, normalize casing)
  const normalizeGrantId = (id) => {
    if (!id) return null;
    return String(id).toLowerCase().replace(/\/$/, '');
  };
  ...
  const existingGrantIds = new Set(
    existingGrants
      .map(g => normalizeGrantId(g.id || g.grantId))
      .filter(id => id !== null)
  );
  const tableName = process.env.NOFO_METADATA_TABLE_NAME;
  ...
  console.log(`[Timing] Knowledge base retrieval completed in ${kbDuration}ms`);
  ...
  for (const result of ragResults) {
    ...
    // Normalize grant ID for comparison
    const normalizedGrantId = normalizeGrantId(grantId);
    if (!normalizedGrantId) continue;
    
    // Skip if already displayed from filter results
    if (existingGrantIds.has(normalizedGrantId)) {
      skippedAlreadyShown++;
      continue;
    }
    ...
    // Keep highest score for each new grant (use normalized ID as key)
    if (!newGrantMap.has(normalizedGrantId) || result.score > newGrantMap.get(normalizedGrantId).score) {
      newGrantMap.set(normalizedGrantId, { grantId, score: result.score });
    }
  }
  
  const processDuration = Date.now() - processStartTime;
  const newGrantEntries = Array.from(newGrantMap.entries());
  console.log(`[RAG Search] Found ${newGrantEntries.length} NEW grants`);
  console.log(`[RAG Search] Skipped: ${skippedAlreadyShown} already displayed, ${skippedBlankMetadata} blank metadata`);
  console.log(`[Timing] Result processing completed in ${processDuration}ms`);
  
  if (newGrantEntries.length === 0) {
    const totalDuration = Date.now() - functionStartTime;
    console.log(`[RAG Search] No additional grants found - done`);
    console.log(`[Timing] Total RAG search time: ${totalDuration}ms`);
    return [];
  }
  
  // Log the new grants found
  console.log(`[RAG Search] New grants from semantic search:`);
  newGrantEntries.forEach(([normalizedId, info], idx) => {
    console.log(`  ${idx + 1}. ${info.grantId} | RAG Score: ${info.score?.toFixed(4) || 'N/A'}`);
  });
  ...
  for (const [normalizedId, info] of newGrantEntries) {
    const grantId = info.grantId;
  ...
  const summaryDuration = Date.now() - summaryStartTime;
  console.log(`[Timing] RAG summary fetching completed in ${summaryDuration}ms`);
  
  const totalDuration = Date.now() - functionStartTime;
  console.log(`[RAG Search] Returning ${enhancedRagGrants.length} additional grants from semantic search`);
  console.log(`[Timing] Total RAG search time: ${totalDuration}ms (KB=${kbDuration}ms, Process=${processDuration}ms, Summary=${summaryDuration}ms)`);
