...
async function findMatchingGrantsWithKB(query, userPreferences = {}) {
  try {
    ...
    const combinedResults = await combineAndDeduplicateResults(
      filteredResults, 
      ragResults, 
      filters, 
...
async function combineAndDeduplicateResults(filteredResults, ragResults, activeFilters = {}, tableName = null) {
  ...
  for (const result of ragResults) {
    ...
    if (hasActiveFilter && tableName) {
      ...
      if (hasBlankMetadata) {
        console.log(`Skipping grant ${grantId} from RAG results - has blank/null ${activeFilters.category ? 'category' : 'agency'}`);
