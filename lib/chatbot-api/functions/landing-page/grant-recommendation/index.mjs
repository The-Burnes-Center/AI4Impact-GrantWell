 ...
 * Returns grants with their category/agency info for debugging
 */
async function searchGrantsByName(searchTerm, tableName) {
  ...
  try {
    // Scan DynamoDB for grants - include category and agency for logging
    const command = new ScanCommand({
      TableName: tableName,
      ProjectionExpression: 'nofo_name, grant_name, category, agency'
    });
    ...
    (response.Items || []).forEach(item => {
      ...
      if (nofoName.toLowerCase().includes(normalizedSearch) || 
        ...
        console.log(`  [Name Match] ${grantName} | Category: ${category || 'blank'} | Agency: ${agency || 'blank'}`);
        matches.push({
          grantId: nofoName,
          grantName: grantName,
          category: category,
