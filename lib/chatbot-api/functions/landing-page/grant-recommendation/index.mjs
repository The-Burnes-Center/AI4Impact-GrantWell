...
/**
 * Get filtered grants immediately (category/agency filter only - fast path)
 */
async function getFilteredGrantsImmediate(query, userPreferences = {}) {
  ...
  const hasFilters = !!(filters.category || filters.agency);
  
  if (!hasFilters) {
    console.log(`[Immediate Filter] No category/agency filters detected - skipping immediate results`);
    return { filteredGrants: [], filters, hasFilters: false };
  }
  
  // Get filtered grants from KB
  let categoryResults = [];
  let agencyResults = [];
  ...
  const processResults = (results, source) => {
    for (const result of results) {
      ...
      if (grantId && !grantMap.has(grantId)) {
        grantMap.set(grantId, { ...result, source: 'db_filter' });
      }
    }
  };
  
  processResults(categoryResults, 'category');
  processResults(agencyResults, 'agency');
  
  const combinedResults = Array.from(grantMap.values());
  console.log(`[Immediate Filter] Found ${combinedResults.length} unique grants from filters`);
  
  // Extract grant info
  const grantInfos = extractGrantInfoFromKBResults(combinedResults);
  grantInfos.forEach(g => g.source = 'db_filter');
  
  // Enhance with keyword matching
  const enhancedGrants = enhanceWithKeywordMatching(grantInfos, filters.keywords);
  
  // Score with LLM (quick scoring for filtered results)
  const scoredGrants = await analyzeAndScoreGrants(query, enhancedGrants, userPreferences);
  
  // Generate enhanced response for filtered grants
  const enhancedFilteredGrants = [];
  for (const grant of scoredGrants) {
    try {
      ...
      if (summary && summary.status !== 'archived') {
        enhancedFilteredGrants.push({
          ...
          summaryUrl: `nofos/${grant.grantId}/summary.json`,
          source: 'db_filter'
        });
      }
    } catch (err) {
      console.warn(`Error enhancing filtered grant ${grant.grantId}:`, err.message);
    }
  }
  
  console.log(`[Immediate Filter] Returning ${enhancedFilteredGrants.length} enhanced filtered grants`);
...
/**
 * Find grants using RAG only (excludes already-found filtered grants)
 */
async function findMatchingGrantsWithRAGOnly(query, userPreferences = {}, filters = {}, existingGrants = []) {
  console.log(`\n========== RAG SEARCH ==========`);
  console.log(`[RAG Search] Query: "${query}"`);
  console.log(`[RAG Search] Excluding ${existingGrants.length} already-found grants`);
  
  const existingGrantIds = new Set(existingGrants.map(g => g.id || g.grantId));
  const tableName = process.env.NOFO_METADATA_TABLE_NAME;
  const enableDynamoDBCache = process.env.ENABLE_DYNAMODB_CACHE === 'true';
  
  // Get RAG results (unfiltered semantic search)
  const ragResults = await retrieveFromKnowledgeBase(query, null);
  console.log(`[RAG Search] Retrieved ${ragResults.length} chunks from semantic search`);
  
  // Filter out already-found grants and grants with blank metadata
  ...
  for (const result of ragResults) {
    ...
    // Check for blank metadata if filters are active
    if (hasActiveFilter && enableDynamoDBCache && tableName) {
    ...
    if (!newGrantMap.has(grantId) || result.score > newGrantMap.get(grantId).score) {
      newGrantMap.set(grantId, { ...result, source: 'rag' });
    }
  }
  
  const newResults = Array.from(newGrantMap.values());
  console.log(`[RAG Search] Found ${newResults.length} new grants from RAG (after dedup)`);
  ...
  // Extract grant info
  const grantInfos = extractGrantInfoFromKBResults(newResults);
  grantInfos.forEach(g => g.source = 'rag');
  
  // Enhance with keyword matching
  const keywords = filters.keywords || extractKeywords(query);
  const enhancedGrants = enhanceWithKeywordMatching(grantInfos, keywords);
  
  // Score with LLM
  const scoredGrants = await analyzeAndScoreGrants(query, enhancedGrants, userPreferences);
  
  // Generate enhanced response for RAG grants
  const enhancedRagGrants = [];
  for (const grant of scoredGrants) {
    // Only include RAG grants that meet the score threshold
    if (grant.source === 'rag' && grant.matchScore < 80) {
      continue;
    }
    ...
    try {
      ...
      console.warn(`Error enhancing RAG grant ${grant.grantId}:`, err.message);
    }
  }
  
  console.log(`[RAG Search] Returning ${enhancedRagGrants.length} enhanced RAG grants`);
