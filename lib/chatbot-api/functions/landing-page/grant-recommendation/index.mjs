...
/**
 * Filter grants by category or agency using DynamoDB
 * Keyword is interpreted as either a category name or agency name
 */
async function filterGrantsByKeyword(keyword, category = null, agency = null) {
  ...
  try {
    ...
    // Check if keyword itself is a valid category (case-insensitive)
    const keywordAsCategory = VALID_CATEGORIES.find(
      cat => cat.toLowerCase() === keywordLower
    );
    
    // Determine effective category and agency filters
    const effectiveCategory = category || keywordAsCategory || null;
    const effectiveAgency = agency || (keywordLower && !keywordAsCategory ? keyword : null);
    
    // If no category or agency filter, return empty (shouldn't happen for simple searches)
    if (!effectiveCategory && !effectiveAgency) {
      console.log('No category or agency filter specified, returning empty results');
      return [];
    }
    ...
    if (effectiveCategory) {
      ...
      // Default to StatusIndex for active grants (when only agency filter)
      queryParams = {
        TableName: tableName,
    ...
    // Filter in-memory by agency only (category already filtered by query)
    for (const grant of activeGrants) {
      const grantAgency = (grant.agency || '').toLowerCase().trim();
      
      let matchesAgency = true;
      
      // Check agency match if agency filter is specified
      if (effectiveAgency) {
        if (!grantAgency) {
          matchesAgency = false;
        } else {
          const agencyLower = effectiveAgency.toLowerCase().trim();
      ...
      if (matchesAgency) {
        matches.push({
          ...
          matchScore: 100,
          matchReason: `Grant matches search criteria${effectiveCategory ? ` (Category: ${effectiveCategory})` : ''}${effectiveAgency ? ` (Agency: ${effectiveAgency})` : ''}`,
    ...
    console.log(`Found ${matches.length} grants matching filters - keyword: ${keyword}, category: ${effectiveCategory}, agency: ${effectiveAgency}`);
