...
async function getFilteredGrantsImmediate(query, userPreferences = {}) {
  ...
  // 1. Get grants matching category from KB
  if (filters.category) {
    const categoryFilter = buildKBMetadataFilter({ category: filters.category });
    const categoryResults = await retrieveFromKnowledgeBase(query, categoryFilter);
    ...
    for (const result of categoryResults) {
      ...
      if (grantId && !grantMap.has(grantId)) {
        grantMap.set(grantId, { grantId, score: result.score, source: 'category', matchReason: `Category: ${filters.category}` });
      }
    }
  }
  
  // 2. Get grants matching agency from KB
  if (filters.agency) {
    const agencyFilter = buildKBMetadataFilter({ agency: filters.agency });
    const agencyResults = await retrieveFromKnowledgeBase(query, agencyFilter);
    ...
    for (const result of agencyResults) {
      const grantId = extractGrantIdFromResult(result);
      if (grantId && !grantMap.has(grantId)) {
        grantMap.set(grantId, { grantId, score: result.score, source: 'agency', matchReason: `Agency: ${filters.agency}` });
      }
    }
  }
    ...
    try {
      ...
      if (summary && summary.status !== 'archived') {
        enhancedFilteredGrants.push({
          ...
          keyRequirements: [],
          summaryUrl: `nofos/${grantId}/summary.json`,
          source: 'db_filter'
        });
      }
    } catch (err) {
...
async function searchGrantsByName(searchTerm, tableName) {
  ...
  try {
    // Scan DynamoDB for grants (we already have the cache from category/agency scans)
    const command = new ScanCommand({
      TableName: tableName,
      ProjectionExpression: 'nofo_name, grant_name'
    });
...
async function findMatchingGrantsWithRAGOnly(query, userPreferences = {}, filters = {}, existingGrants = []) {
  ...
  console.log(`[RAG Search] Found ${newGrantIds.length} NEW grants (skipped ${skippedAlreadyShown} already displayed)`);
