    ...
    for (const match of categoryMatches) {
      if (!grantMap.has(match.grantId)) {
        grantMap.set(match.grantId, { 
          ...
          grantType: match.grantType || 'unknown'
        });
      }
    }
    ...
    try {
      ...
      if (summary && summary.status !== 'archived') {
        enhancedFilteredGrants.push({
          ...
          source: 'db_filter',
          grantType: info.grantType || summary?.grant_type || 'unknown'
        });
      }
    } catch (err) {
  ...
  try {
    ...
    (response.Items || []).forEach(item => {
      ...
      matches.push({
        ...
        grantType: unmarshalled.grant_type || 'unknown'
      });
    });
...
async function searchGrantsByName(searchTerm, tableName) {
  ...
  try {
    ...
    (response.Items || []).forEach(item => {
      ...
      const grantType = unmarshalled.grant_type || 'unknown';
        ...
        if (tableName) {
          try {
            ...
            if (dbResponse.Item) {
              ...
              grantType = dbItem.grant_type || 'unknown';
              console.log(`  [RAG Grant Type] ${grantId} -> ${grantType}`);
            } else {
              console.log(`  [RAG Grant Type] ${grantId} -> not found in DynamoDB`);
            }
          } catch (dbErr) {
