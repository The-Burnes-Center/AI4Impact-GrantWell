...
async function findMatchingGrantsWithKB(query, userPreferences = {}) {
  try {
    ...
    // 1. Build metadata filter for KB query
    const metadataFilter = buildKBMetadataFilter(filters);
    
    // 2. Retrieve relevant documents from Knowledge Base with metadata filtering
    const kbResults = await retrieveFromKnowledgeBase(query, metadataFilter);
    if (!kbResults || kbResults.length === 0) {
      console.log('No results from Knowledge Base for query:', query);
      return [];
    }
    
    // 3. Extract grant information from the KB results
    const grantInfos = extractGrantInfoFromKBResults(kbResults);
    
    // 4. Enhance results with keyword matching boost
    const enhancedGrants = enhanceWithKeywordMatching(grantInfos, filters.keywords);
    
    // 5. Use Bedrock to analyze and score the grants against the user query
