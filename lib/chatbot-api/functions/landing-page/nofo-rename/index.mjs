...
import { canSafelyRollbackRenameClaim } from './rollback-safety.mjs';
...
export { canSafelyRollbackRenameClaim };
...
export const handler = async (event) => {
  ...
  // Becomes true once we start deleting the source S3 prefix. After that, rolling back the
  // claimed newName row risks making the grant disappear entirely from the dashboard.
  ...
  try {
    ...
    // Point of no return for claim rollback: source objects are about to disappear.
    sourceMutated = true;
    for (const object of objects) {
    ...
    // Only release the claimed name while the source is still intact. After source mutation,
    // deleting newName would orphan S3 under the new prefix and can wipe the grant from the
    // dashboard entirely if the old DynamoDB row is also gone or incomplete.
    if (canSafelyRollbackRenameClaim(claimedNewRow, sourceMutated)) {
      try {
        await dynamoClient.send(new DeleteItemCommand({
          TableName: METADATA_TABLE,
    ...
    } else if (claimedNewRow && sourceMutated) {
      console.error(
        `Rename failed after source mutation; leaving metadata row '${newName}' in place to avoid data loss`
      );
    }
