      ...
      try {
        ...
        for (const opportunity of opportunities) {
          ...
          try {
            ...
            if (exists) {
              // NOFO exists - check if grant_type needs to be backfilled to 'federal'
              ...
              if (enableDynamoDBCache && tableName) {
                try {
                  ...
                  // Check if grant_type is missing
                  ...
                  if (existingItem.Item) {
                    ...
                    // If grant_type is not set, backfill it to 'federal'
                    if (!item.grant_type) {
                      const now = new Date().toISOString();
                      const updateCommand = new UpdateItemCommand({
                        TableName: tableName,
                        Key: marshall({ nofo_name: opportunityTitle }),
                        UpdateExpression: 'SET grant_type = :grant_type, updated_at = :updated_at',
                      ...
                      console.log(`Backfilled grant_type=federal for existing NOFO: ${opportunityTitle}`);
                    }
                  }
                } catch (error) {
                  console.warn(`Failed to backfill grant_type for ${opportunityTitle}:`, error.message);
            ...
            if (enableDynamoDBCache && tableName && extension === 'pdf') {
              try {
                ...
                console.log(`Successfully wrote NOFO metadata to DynamoDB for ${opportunityTitle} with grant_type=federal`);
              } catch (error) {
