...
const CLAUDE_MODEL_ID = process.env.HAIKU_MODEL_ID;

/** Normalize NOFO folder name: slashes → dashes so S3 path matches DynamoDB nofo_name */
function normalizeNofoFolderName(name) {
...
Return ONLY the JSON object, no additional text or explanation.`;
    ...
    if (result.nofoIndex >= 1 && result.nofoIndex <= attachments.length) {
      ...
      console.error(`Invalid NOFO index ${result.nofoIndex} returned by Bedrock`);
      return null;
    }
  } catch (error) {
 ...
 * Uses normalized name (slashes → dashes) for consistency; also checks original for legacy data.
 */
async function nofoExistsInS3(opportunityTitle, s3Client) {
  try {
    const normalized = normalizeNofoFolderName(opportunityTitle);
    const prefixesToCheck = [normalized];
    if (normalized !== opportunityTitle) prefixesToCheck.push(opportunityTitle);
    for (const prefix of prefixesToCheck) {
      ...
      if (result.Contents && result.Contents.length > 0) return true;
    }
    return false;
  } catch (error) {
        ...
        for (const opportunity of opportunities) {
          ...
          try {
            ...
            if (exists) {
              ...
              const existingFolderName = normalizeNofoFolderName(opportunityTitle);
              ...
              if (enableDynamoDBCache && tableName) {
                try {
                  ...
                  const getCommand = new GetItemCommand({
                    TableName: tableName,
                    Key: marshall({ nofo_name: existingFolderName }),
                  });
                  ...
                  if (existingItem.Item) {
                    ...
                    const needsExpirationDate = !item.expiration_date;
                    ...
                    if (needsAgency || needsCategory || needsExpirationDate) {
                      ...
                      if (details) {
                        ...
                        if (needsExpirationDate) {
                          const normalizedDate = details.close_date ? normalizeDateToYYYYMMDD(details.close_date) : null;
                          if (normalizedDate) {
                            updateExpressions.push('expiration_date = :expiration_date');
                            expressionAttributeValues[':expiration_date'] = normalizedDate;
                          }
                          updateExpressions.push('is_rolling = :is_rolling');
                          expressionAttributeValues[':is_rolling'] = String(!details.close_date);
                        }
                      }
                    }
                    ...
                    if (updateExpressions.length > 0) {
                      ...
                      console.log(`Backfilled metadata for existing NOFO: ${existingFolderName} - Fields updated: ${updateExpressions.join(', ')}`);
                    }
                  }
                } catch (error) {
                  console.warn(`Failed to backfill metadata for ${existingFolderName}:`, error.message);
                }
              }
            ...
            const folderName = normalizeNofoFolderName(opportunityTitle);
            let s3Key;
            let contentType;
            let fileName;
            let status = 'success';
            ...
            if (extension === 'html') {
              ...
              // Write agency and category to DynamoDB for HTML files (the NOFO pipeline will add rest of data)
              const tableName = process.env.NOFO_METADATA_TABLE_NAME;
              const enableDynamoDBCache = process.env.ENABLE_DYNAMODB_CACHE === 'true';
              ...
              if (enableDynamoDBCache && tableName) {
                try {
                  ...
                  // Check if item already exists (use folderName for consistency)
                  ...
                  if (details.category && (!existingItem || !existingItem.agency || !existingItem.category)) {
                    ...
                    const isRolling = !details.close_date;
                    ...
                    console.log(`Successfully wrote NOFO metadata to DynamoDB for HTML ${folderName} - Agency: ${details.agency_name}, Category: ${details.category}`);
                  }
                } catch (error) {
                  console.error(`Failed to write to DynamoDB for HTML ${folderName}:`, error);
                  // Continue execution - S3 is the source of truth, and the NOFO pipeline will handle it
                }
              }
            ...
            // Write to DynamoDB if enabled (for PDF files - HTML files write agency/category above, the NOFO pipeline adds rest)
            const tableName = process.env.NOFO_METADATA_TABLE_NAME;
            const enableDynamoDBCache = process.env.ENABLE_DYNAMODB_CACHE === 'true';
            ...
            if (enableDynamoDBCache && tableName && extension === 'pdf') {
              try {
                ...
                if (!details.category) {
                  ...
                  await dynamoClient.send(putCommand);
                  console.log(`Successfully wrote NOFO metadata to DynamoDB for ${folderName} - Agency: ${details.agency_name}, Category: ${details.category}, Expiration: ${expirationDate || 'none'}`);
                }
              } catch (error) {
                console.error(`Failed to write to DynamoDB for ${folderName}:`, error);
                // Continue execution - S3 is the source of truth, and the NOFO pipeline will handle it
              }
            }
