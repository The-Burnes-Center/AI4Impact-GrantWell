...
async function fetchOpportunityDetails(opportunityId) {
  try {
    ...
    // Extract category from funding_categories array
    let category = 'Other';
              ...
              if (enableDynamoDBCache && tableName) {
                try {
                  ...
                  if (existingItem.Item) {
                    ...
                    // Check and backfill agency and category
                    const needsAgency = !item.agency || item.agency === 'Unknown';
                    const needsCategory = !item.category || item.category === 'Other';
                    
                    if (needsAgency || needsCategory) {
                      // Fetch opportunity details to get agency and category
                      ...
                      if (details) {
                        ...
                        if (needsCategory && details.category && details.category !== 'Other') {
            ...
            if (enableDynamoDBCache && tableName && extension === 'pdf') {
              try {
                ...
                const item = {
                  ...
                  category: details.category || 'Other', // Extract and map from API response
                  created_at: now,
                  updated_at: now,
                };
                ...
                console.log(`Successfully wrote NOFO metadata to DynamoDB for ${opportunityTitle} - Agency: ${details.agency_name}, Category: ${details.category}`);
              } catch (error) {
