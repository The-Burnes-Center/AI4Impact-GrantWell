  ...
  const bedrockCommand = new InvokeModelCommand({
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "user",
          content: `You are an AI assistant tasked with writing 5–15 highly specific, strategic questions tailored to the NOFO (Notice of Funding Opportunity) below. These questions will be used by another AI to help generate a complete, high-quality grant proposal.

IMPORTANT INSTRUCTIONS:

1. Write between 5 and 15 questions (aim for 8–10).
2. **Each question must be unique** — do not repeat or rephrase the same question in different ways.
...
4. Questions must be concise and directly aligned with this specific NOFO’s content, terminology, and evaluation criteria.
...
function createBedrockCommand(contents, isMerged = false) {
  ...
  * Keep **Eligibility Criteria**, **Required Documents**, and **Key Deadlines** concise — extract only the **most important** items per category.
  * For **Project Narrative Sections**, extract **all required narrative components mentioned in the NOFO**, including technical, administrative, and optional narrative elements — but **exclude sections that require applicant-specific financial or budget content** (e.g., budget narrative, budget tables, or cost breakdowns).
  * For each item, provide a description of exactly 2-3 sentences (not more).
  * Prioritize the most critical information that applicants need to know.
  * **Project Narrative Sections must cover every required section explicitly mentioned** (e.g., problem statement, project goals, methodology, timeline, stakeholder engagement, sustainability plan, evaluation plan, capacity statement, organizational background, etc.).
  * Combine related items when possible rather than listing them separately.
  * If there are any relevant websites or links provided in the NOFO document associated with an item, include them within the description using markdown format (e.g., [EPA Guidelines](https://www.epa.gov)).
  * Do not create a separate section for links.
  * Avoid overly granular details — focus on high-level requirements.
  * Maintain clean structure and avoid duplicating information.

  Format the output in JSON format. The expected format is as follows:
  {
...
${contents}`
    : `You are an AI assistant that helps categorize NOFO documents into various categories. Here is a NOFO document. Please extract the name of the grant and categorize the information into 4 categories: Eligibility Criteria, Required Documents, Project Narrative Sections, and Key Deadlines.

IMPORTANT GUIDELINES:
- Keep each category concise - extract only the 2-3 MOST IMPORTANT items per category
- For each item, provide a description of exactly 2-3 sentences (not more)
- Prioritize the most critical information that applicants need to know
- Combine related items when possible rather than listing them separately
- **If there are any relevant websites or links provided in the NOFO document associated with an item, include them within the description using markdown format (e.g., [EPA Website](https://www.epa.gov)).**
- Do not create a separate section for links
- Avoid overly granular details - focus on high-level requirements
...
Format the output in JSON format. The expected format is as follows:
{
  ...
  "EligibilityCriteria": [
    { "item": "Eligibility Item 1", "description": "Explanation of what Eligibility Item 1 means in 2-3 sentences, including any relevant [EPA Guidelines](https://www.epa.gov)." },
    { "item": "Eligibility Item 2", "description": "Explanation of what Eligibility Item 2 means in 2-3 sentences, including any relevant [EPA Guidelines](https://www.epa.gov)." }
  ],
  "RequiredDocuments": [
    { "item": "Document Name 1", "description": "Explanation of what Document Name 1 means in 2-3 sentences, including any relevant [EPA Requirements](https://www.epa.gov)." },
    { "item": "Document Name 2", "description": "Explanation of what Document Name 2 means in 2-3 sentences, including any relevant [EPA Requirements](https://www.epa.gov)." }
  ],
  "ProjectNarrativeSections": [
    { "item": "Section Name 1", "description": "Explanation of what Section Name 1 means in 2-3 sentences, including any relevant [EPA Guidelines](https://www.epa.gov)." },
    { "item": "Section Name 2", "description": "Explanation of what Section Name 2 means in 2-3 sentences, including any relevant [EPA Guidelines](https://www.epa.gov)." }
  ],
  "KeyDeadlines": [
    { "item": "Deadline 1", "description": "Explanation of what Deadline 1 means in 2-3 sentences, including any relevant [Submission Guidelines](https://www.epa.gov)." },
    { "item": "Deadline 2", "description": "Explanation of what Deadline 2 means in 2-3 sentences, including any relevant [Submission Guidelines](https://www.epa.gov)." }
  ]
}
