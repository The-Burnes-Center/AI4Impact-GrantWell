import {
    ...
    try {
        ...
        if (documentChunks.length > 1) {
            ...
            if (requirementsJSON) {
              ...
              console.error("Failed to extract JSON from the model's response: ", completionExtract)
            }
        } else {
            mergedSummary = JSON.parse(summaries[0]);
        }
  
        // Generate questions for each Project Narrative Section
        let questionsData = null;
        try {
          questionsData = await generateQuestionsForSections(mergedSummary, bedrockClient);
  ...
  // Generate questions for each Project Narrative Section
  async function generateQuestionsForSections(mergedSummary, bedrockClient) {
    if (!mergedSummary.ProjectNarrativeSections || mergedSummary.ProjectNarrativeSections.length === 0) {
      console.log("No Project Narrative Sections found to generate questions");
      return null;
    }
    
    // Include both section name and description in the prompt for better context
    const sectionsData = JSON.stringify(mergedSummary.ProjectNarrativeSections, null, 2);
    
    // Create prompt for Bedrock to generate questions
    const bedrockCommand = new InvokeModelCommand({
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `Generate 1-5 specific questions for each Project Narrative Section that will help collect information from grant applicants. 
          These questions should:
          1. Help extract all relevant details needed to write a compelling narrative for each section
          2. Be specific to the requirements described in each section
          3. Cover all required components mentioned in the section description
          4. Elicit responses that will directly address the NOFO evaluation criteria
          5. Be clear and straightforward for non-technical users to understand
          
          For example, if a section is about "Project Goals", you might ask:
          1. What are the primary objectives of your project?
          2. How do these goals align with the funding agency's priorities?
          3. What specific metrics will you use to measure success?
          ...
          Here are the Project Narrative Sections from the NOFO:
          ${sectionsData}`
        }],
        max_tokens: 4000,
    ...
    try {
      ...
      // Extract JSON from response - improve regex to be more robust
      ...
      if (jsonMatch) {
        try {
          ...
          // Attempt a more forgiving extraction approach if strict JSON parsing fails
          const sanitizedJson = jsonMatch[0].replace(/[\u0000-\u001F]+/g, " ").trim();
          return JSON.parse(sanitizedJson);
        }
      } else {
        console.error("Failed to extract questions JSON from the model's response");
        return null;
      }
    } catch (error) {
