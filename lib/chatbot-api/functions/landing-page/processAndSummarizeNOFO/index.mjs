...
export const handler = async (event) => {
  ...
  for (const sqsRecord of event.Records) {
    try {
      ...
      // Fetch agency and category from DynamoDB (already populated by automated scraper)
      let agency = null;
      let category = null;
      ...
      if (enableDynamoDBCache && tableName) {
        try {
          ...
          if (existingItem.Item) {
            ...
            console.log(`Fetched agency=${agency}, category=${category} from DynamoDB for ${nofoName}`);
          } else {
            console.log(`No existing DynamoDB entry for ${nofoName} - agency/category will be null`);
          }
        } catch (error) {
          console.warn(`Could not fetch agency/category from DynamoDB for ${nofoName}:`, error.message);
        }
      }
      ...
      if (enableDynamoDBCache && tableName) {
        try {
          ...
          // Preserve expiration_date from scraper (API) if it exists, otherwise use AI-extracted date
          // This ensures scraped NOFOs keep their API-set expiration_date, while manually uploaded NOFOs get AI-extracted date
