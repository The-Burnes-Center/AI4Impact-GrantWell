...
async function fetchFromDynamoDB(tableName) {
  ...
  try {
    ...
    // Query processing NOFOs (in-progress pipeline)
    const processingQuery = new QueryCommand({
      TableName: tableName,
      IndexName: 'StatusIndex',
      KeyConditionExpression: '#status = :status',
      ExpressionAttributeNames: { '#status': 'status' },
      ExpressionAttributeValues: marshall({ ':status': 'processing' }),
    });
    ...
    // Combine all NOFOs
    const allNofos = [...activeNofos, ...archivedNofos, ...processingNofos];

    console.log(`Retrieved ${allNofos.length} NOFOs from DynamoDB (${activeNofos.length} active, ${archivedNofos.length} archived, ${processingNofos.length} processing)`);
