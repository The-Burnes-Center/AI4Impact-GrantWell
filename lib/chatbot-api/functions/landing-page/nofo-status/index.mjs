...
async function updateDynamoDB(tableName, nofoName, status, isPinned, expirationDate, grantType, category) {
  ...
  try {
    ...
    if (existingItem.Item) {
      ...
      if (!category) {
        throw new Error('Category is required when creating a new NOFO entry');
      }
...
export const handler = async (event) => {
  ...
  try {
    ...
    const { nofoName, status, isPinned, expirationDate, grantType } = requestBody;
