 ...
 * Valid grant types
 */
const VALID_GRANT_TYPES = ['federal', 'state', 'quasi', 'philanthropic', 'unknown'];

/**
 * Update NOFO metadata in DynamoDB
 */
async function updateDynamoDB(tableName, nofoName, status, isPinned, expirationDate, grantType, category) {
  ...
  try {
    ...
    if (existingItem.Item) {
      ...
      if (category !== undefined) {
        // Category is mandatory - validate it's not null or empty
        if (!category) {
...
export const handler = async (event) => {
  ...
  try {
    ...
    const { nofoName, status, isPinned, expirationDate, grantType, category } = requestBody;
    ...
    if (grantType && !VALID_GRANT_TYPES.includes(grantType)) {
      return {
        statusCode: 400,
        headers: { 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ message: "Grant type must be 'federal', 'state', 'quasi', 'philanthropic', or 'unknown'" }),
      };
    }
