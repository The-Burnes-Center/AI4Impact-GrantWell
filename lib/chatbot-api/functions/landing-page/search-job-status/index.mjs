...
export const handler = async (event) => {
  ...
  try {
    ...
    // Return job status
    // If allGrants exists (complete deduplicated results), use that; otherwise return separate arrays
    const responseBody = {
      jobId: job.jobId,
      status: job.status,
    ...
    // If complete deduplicated results are available, include them
    if (job.allGrants && Array.isArray(job.allGrants)) {
      responseBody.allGrants = job.allGrants;
      console.log(`[Job Status] Returning ${job.allGrants.length} complete deduplicated grants`);
    }
