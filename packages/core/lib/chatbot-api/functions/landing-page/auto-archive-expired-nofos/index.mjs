  ...
  try {
    ...
    for (const nofo of allActiveNofos) {
      ...
      try {
        ...
        // Support both YYYY-MM-DD format and ISO string format
        let expirationDate;
        if (/^\d{4}-\d{2}-\d{2}$/.test(nofo.expiration_date)) {
          // YYYY-MM-DD format - parse as UTC date
          expirationDate = new Date(`${nofo.expiration_date}T23:59:59Z`);
        } else {
          // ISO string format (old format)
          expirationDate = new Date(nofo.expiration_date);
        }
        
        if (isNaN(expirationDate.getTime())) {
          stats.errors++;
          console.error(`Invalid expiration_date for ${nofo.nofo_name}: ${nofo.expiration_date}`);
          continue;
        }
