      ...
      Item: marshall(
        {
          ...
          state,
          status: "active",
          // Lets the list endpoint hide the federal original from this state, so its users see one
          // row instead of the copy sitting next to its parent.
          promoted_from: sourceRow.nofo_name,
          ...
          isPinned: "false",
          // Inherit the original's age: forking is not a new grant reaching the state, and a fresh
          // stamp would light up the "New" badge on a grant users may have seen for weeks.
